...
Self-service swaps, transfers and vacancy claims retain the source roster version and each manager approval stage. The current published assignments stay in force between stages; final revalidation and publication of every affected roster run as one checked company transaction. Manager rights cover the receiving employee as well as the original roster. Raw attendance events and colleague attendance results are restricted independently of permission to see the schedule; historical results use the employment scope effective on their original work date.
